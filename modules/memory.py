...
class Memory(Module):
    def __init__(self, signals, enabled=True):
        ...
        print(f"память содержит {self.collection.count()} воспоминаний")
        if self.collection.count() == 0:
            print("память пуста, инициализация из файла...")
    ...
    def get_prompt_injection(self):
        ...
        self.prompt_injection.text = f"{AI_NAME} знает следующее из своих воспоминаний:\n"
        for i in range(len(memories["ids"][0])):
            self.prompt_injection.text += memories['documents'][0][i] + "\n"
        self.prompt_injection.text += "Конец воспоминаний.\n"
    ...
    async def run(self):
        while not self.signals.terminate:
            ...
            if len(self.signals.history) - self.processed_count >= 20:
                print("Память: Создание новых воспоминаний...")
